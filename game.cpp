...
void Game::update(Uint32 dt)
{
    ...
    else
    {
        ...
        for(auto player : m_players)
            for(auto enemy : m_enemies)
            {
                //kiểm tra sự va chạm của xe tăng địch với người chơi
                checkCollisionTwoTanks(player, enemy, dt);
                //kiểm tra sự va chạm của đạn của người chơi với địch
                checkCollisionPlayerBulletsWithEnemy(player, enemy);

                //kiểm tra sự va chạm của đạn của người chơi với đạn của đối thủ
                for(auto bullet1 : player->bullets)
                     for(auto bullet2 : enemy->bullets)
                            checkCollisionTwoBullets(bullet1, bullet2);
            }

        //kiểm tra sự va chạm của đạn của đối thủ với người chơi
        for(auto enemy : m_enemies)
            for(auto player : m_players)
        ...
        //Kiểm tra va chạm của xe tăng với bản đồ
...
void Game::checkCollisionTankWithLevel(Tank* tank, Uint32 dt)
{
    ...
    //========================Va chạm với các đối tượng trên bản đồ========================
    ...
    //Hình chữ nhật ở phía dưới của bản đồ
    outside_map_rect.x = 0;
...
void Game::checkCollisionBulletWithLevel(Bullet* bullet)
{
    ...
    //========================Va chạm với các yếu tố trên bản đồ========================
