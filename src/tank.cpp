...
Tank::Tank() //Khởi rạo xe tăng ở vị trí và mặc định
...
void Tank::update(Uint32 dt) //Cập nhật vị trí xử lý va chạm quản lý thời gian các hiệu ứng, xóa đạn đã bị hủy
{
    if(to_erase) return;
    if(testFlag(TSF_LIFE))
    {
        ...
        {
            //Di chuyển xe tăng nếu ko bị dừng hoặc đóng băng
            switch (direction)
            {
            case D_UP:
                pos_y -= speed * dt;
    ...
    //Xử lý trạng thái trượt trên băng
    ...
    if(m_sprite->frames_count > 1 && (testFlag(TSF_LIFE) ? speed > 0 : true)) //không có hoạt hình nếu xe tăng không cố gắng đi
    {
...
Bullet* Tank::fire() //Tạo đạn thêm, ds đạn và đặt tốc độ đạn
{
    if(!testFlag(TSF_LIFE)) return nullptr;
    if(bullets.size() < m_bullet_max_size)
    {
        //Tạm thời gán vị trí ban đầu vì chưa xác định được kích thước của đạn
        ...
        bullet->update(0); //thay đổi vị trí dest_rect
        return bullet;
    }
    return nullptr;
}
...
{
    if(intersect_rect.w > intersect_rect.h) // va chạm từ trên xuống hoặc từ dưới lên
    {
...
void Tank::respawn()
{
    ...
    m_flags = TSF_LIFE;
    update(0);
    m_flags = TSF_CREATE; //Reset tất cả các cờ khác
    //Cập nhật vị trí hình chữ nhật va chạm sau khi gọi hàm update
    collision_rect.x = 0;
    collision_rect.y = 0;
    collision_rect.h = 0;
    collision_rect.w = 0;
