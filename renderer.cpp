...
void Renderer::clear()
{
    ...
    SDL_RenderClear(m_renderer); //Xóa bộ đệm phía sau
}

void Renderer::flush()
{
    SDL_RenderPresent(m_renderer); //Trao đổi bộ đệm
}

void Renderer::drawObject(const SDL_Rect *texture_src, const SDL_Rect *window_dest)
{
    SDL_RenderCopy(m_renderer, m_texture, texture_src, window_dest); //Vẽ lên bộ đệm phía sau
}
